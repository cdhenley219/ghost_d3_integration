let $section;
let $guess__container ;
let $guess__svg;
let $instruction2;
let $warningMsg;
let $interpretText;
let $submitBtn ;
let $answer__container;
let $answer__svg;
...
function initialize() {
	...
	$section = d3.select('.guessMovement');
	$guess__container = $section.select('.guess__figure');
	$guess__svg = $guess__container.select('svg');
	$instruction2 = $guess__container.select('.instruction2');
	$warningMsg = $guess__container.select('.warningMsg');
	$interpretText = $guess__container.select('.interpretText');
	$submitBtn = $section.select('.submitBtn');
	$answer__container = $section.select('.answer__figure');
	$answer__svg = $answer__container.select('svg');
	...
	answerData = [{levels: 0, share: 0.6},
				 	{levels: 1, share: 11.1},
					{levels: 2, share: 44.2},
					{levels: 3, share: 29.5},
					{levels: 4, share: 14.5}];

	guessData = [{levels: 0, share: 10},
				 {levels: 1, share: 10},
				 {levels: 2, share: 10},
				 {levels: 3, share: 10},
				 {levels: 4, share: 10}];
...
// dimensions
	margin = {top: 20, bottom: 50, left: 46, right: 0};
	width = 0;
	height = 0;
